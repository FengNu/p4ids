...
control MyIngress(inout headers hdr,
    ...
    table ipv4_exact {
        ...
        const entries = {
            (0x0a000001, 0x0a000101) : ipv4_forward(0x000c29bdb32e, 2);
            (0x0a000101, 0x0a000001) : ipv4_forward(0x005079666801, 1);
        }
        ...
        if (hdr.ethernet.etherType == TYPE_ARP) {
            ...
            if (ipv4_exact.apply().miss) {
                ...
                if (hdr.ethernet.etherType == TYPE_PROBE) {
                    ...
                    log_msg("hdr.ethernet.etherType == TYPE_IPV4");
                    //check egress_spec
                    ...
                    if(mac == 0 && ip == 0) {
                        log_msg("forward packet add trace header");
                        // forward packet add trace header
                        hdr.ethernet.etherType = TYPE_TRACE;
                        hdr.elastic.setValid();
                        hdr.elastic.elastic = hdr.elastic.elastic | trace_tmp;

                    } else if (hdr.ethernet.dstAddr != mac || hdr.ipv4.dstAddr != ip) {
                        send_alert(WRONG_DST);
                    }

                } else if (hdr.ethernet.etherType == TYPE_TRACE) {
                    log_msg("hdr.ethernet.etherType == TYPE_TRACE");
                    if (trace_tmp & hdr.elastic.elastic == 1) {
                        send_alert(LOOP);
                    } else {
                        // continue
                        bit<80> host_info_tmp2 = 0;
                        host_table.read(host_info_tmp2, (bit<32>)standard_metadata.egress_spec);
                        macAddr_t mac2 = 0;
                        ip4Addr_t ip2 = 0;
                        get_host_info(host_info_tmp2, mac2, ip2);
                        if(mac2 == 0 && ip2 == 0) {
                            // continue forwarding
                            log_msg("continue forwarding");
                            hdr.elastic.elastic = hdr.elastic.elastic | trace_tmp;
                        } else {
                            // to the end
                            log_msg("to the end");
                            ...
                            if (hdr.ethernet.dstAddr != mac2 || hdr.ipv4.dstAddr != ip2) {
                                log_msg("wrong destination");
                                log_msg("hdr.ethernet.dstAddr:{} != mac2:{} || hdr.ipv4.dstAddr:{} != ip2:{}",{hdr.ethernet.dstAddr, mac2, hdr.ipv4.dstAddr, ip2});
                                // wrong destination
                                send_alert(WRONG_DST);
                            }
                        }
