...
def main():
    ...
    parser.add_argument('-p', '--proto-dump-file',
                        ...
                        type=str, action="store", required=False) # the file that save request
    parser.add_argument("-c", '--runtime-conf-file',
                        help="path to input runtime configuration file (JSON)",
                        type=str, action="store", required=False)
    parser.add_argument("-t", '--operations',
                        help="write or read entry operations",
                        type=str,action="store",required=True)
    parser.add_argument("-e", '--table-id',
                        help="read table id",
                        type=str, action="store", required=False)

    args = parser.parse_args()

    if(args.operations == "read"):
        read_switch_table(args.p4runtime_server_addr,args.device_id,args.table_id)
    else:
...
# push p4 compiled json file and entries
def program_switch(addr, device_id, sw_conf_file, workdir, proto_dump_fpath):
    ...
    if target == "bmv2":
        sw = bmv2.Bmv2SwitchConnection(address=addr, device_id=device_id, # Bmv2SwitchConnection extends SwitchConnection
    ...
    try:
        ...
        if target == "bmv2":
            info("Setting pipeline config (%s)..." % sw_conf['bmv2_json']) # p4c compiled json file
            ...
            sw.SetForwardingPipelineConfig(p4info=p4info_helper.p4info, # p4info is in
                                           bmv2_json_file_path=bmv2_json_fpath)# set p4c compiled json file
        else:
