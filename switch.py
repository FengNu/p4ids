...
class SwitchConnection(object):
    ...
    def __init__(self, name=None, address='127.0.0.1:50051', device_id=0,
        ...
        # address is the grpc add of the switch device_id should be same as the id set on the switch, proto_dump_file is to record the requests to the switch
        self.name = name
    ...
    def MasterArbitrationUpdate(self, dry_run=False, **kwargs): # set device id
    ...
    # ser p4info and p4c compiled json file to switch
    def SetForwardingPipelineConfig(self, p4info, dry_run=False, **kwargs):
        device_config = self.buildDeviceConfig(**kwargs)
        request = p4runtime_pb2.SetForwardingPipelineConfigRequest()
        request.election_id.low = 1 # set twice
        request.device_id = self.device_id
        config = request.config

        config.p4info.CopyFrom(p4info) # copy p4info file to request.config
        config.p4_device_config = device_config.SerializeToString() # p4c complied json file
